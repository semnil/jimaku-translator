...
- **Silero VAD**: 512-sample チャンク + 64-sample コンテキスト (入力 [1, 576])。feedQueue は Promise チェーンで直列化、深さ 4 上限でドロップ、`.catch(() => {})` でチェーン断絶防止。30 秒間の非発話継続で LSTM 隠れ状態を自動リセット。500ms の pre-speech リングバッファで発話先頭の立ち上がりを補完。`emitSpeech()` 実行ごとに LSTM 状態をリセット (state を持ち越すと次発話の prob が崩壊するため)。母音飽和で prob が崩壊した場合に備え、silence ブランチでも `speechSampleCount >= maxSpeechSamples` で強制分割を行う (continuationRmsFloor で silenceSamples が 0 に固定されても確実に Whisper Dispatch を発火)
